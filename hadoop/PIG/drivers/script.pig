drivers = LOAD '/user/cloudera/pig/drivers/drivers.csv' USING PigStorage(',');
raw_drivers = FILTER drivers BY $0>1;
drivers_details = FOREACH raw_drivers GENERATE $0 AS driverId, $1 AS name;
timesheet = LOAD '/user/cloudera/pig/drivers/timesheet.csv' USING PigStorage(',');
...
rmf /user/cloudera/pig/drivers;
store join_data into '/user/cloudera/pig/drivers/out/';
